...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { code: @target.code, description: @target.description, measure_id: @target.measure_id } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { code: @target.code, description: @target.description, measure_id: @target.measure_id } }
